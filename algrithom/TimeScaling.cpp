...
OsFlt64 *Time_Scaling(OsFlt64 *input, const OsInt32 Lx,const OsFlt64 time_scaling_ratio){
	...
	{
		memcpy(analysis_frame, input + pin, sizeof(OsFlt64)*frame_length);// ��ȡһ֡����

		for (i = 0; i < NFFT; i++)
			*(analysis_frame + i) *= *(window + i);//�Ӵ�
		...
		for (i = 0; i < NFFT / 2; i++)
		{
			*(this_analysis_phase + i) = atan2(dft_half[i].i, dft_half[i].r);//��λ
			*(sp + i) = sqrt(dft_half[i].r*dft_half[i].r + dft_half[i].i*dft_half[i].i);//��ֵ
			*(delta_phase + i) = *(this_analysis_phase + i) - *(last_analysis_phase + i);//��ȡ֮֡�����λ��
